...
function selM(el, method) {
  ...
  if (method === 'paypal') { initPaiementPayPal(); }
}
<?php if($stripe_pk): ?>
...
<?php endif; ?>
<?php if($paypal_cid): ?>
var paiementPPRendered = false;
function initPaiementPayPal() {
  if (paiementPPRendered || typeof paypal === 'undefined') return;
  paiementPPRendered = true;
  paypal.Buttons({
    createOrder: function(d, a) {
      return a.order.create({purchase_units:[{amount:{value:'<?= number_format((float)$pl['amount'],2,'.','')?>'}}]});
    },
    onApprove: function(d, a) {
      return a.order.capture().then(function() {
        var f = document.getElementById('pmt-form');
